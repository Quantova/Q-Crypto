...
//! Quantova-Chain (ML-KEM + ML-DSA handshake; no X25519).
//!
//! This module implements the ML-KEM-768 parameter set (FIPS 203, security category 3). It covers
//! key generation, encapsulation, and decapsulation, together with the number theoretic transform,
//! the sampling routines, the compression and decompression, and the byte encodings. The H, G, and
//! J functions and the XOF and PRF expansions use SHA3-256, SHA3-512, SHAKE256, and SHAKE128 from
//! the `sha3` module.
...
/// the ciphertext (FIPS 203, Algorithm 17, ML-KEM.Encaps_internal).
...
/// that does not re-encrypt to itself the implicit rejection secret is returned instead
