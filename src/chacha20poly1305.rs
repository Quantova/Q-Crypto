...
/// section 2.8). Returns the ciphertext and the 128-bit tag over the associated data and ciphertext.
/// The nonce must be unique for every message sealed under a given key.
pub fn seal(
    key: &[u8; KEY_BYTES],
...
/// constant time, and return the plaintext only on an exact match. Any tampering with the
/// ciphertext, tag, associated data, nonce, or key yields `None` and no plaintext is revealed.
pub fn open(
    key: &[u8; KEY_BYTES],
    nonce: &[u8; NONCE_BYTES],
    aad: &[u8],
