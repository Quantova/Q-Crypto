...
//! QVM `SLH_VERIFY` opcode.
//!
//! Parameter set SLH-DSA-SHAKE-192s (category 3, small signature). Every hash is a SHAKE256 call
//! from the sha3 module, matching the SHAKE instantiation of FIPS 205 section 11.2.
...
///
/// Returns the secret key (SK.seed, SK.prf, PK.seed, PK.root) and public key (PK.seed, PK.root).
pub fn keygen(
    sk_seed: &[u8; N],
    sk_prf: &[u8; N],
    pk_seed: &[u8; N],
...
///
/// `addrnd` is the additional randomness; passing PK.seed gives the deterministic variant.
...
///
/// `addrnd` is the additional randomness; passing PK.seed gives the deterministic variant. Returns
/// `None` if the context is longer than 255 bytes.
pub fn sign(
    sk: &[u8; SECRET_KEY_BYTES],
    message: &[u8],
    context: &[u8],
