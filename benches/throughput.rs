...
//!
//! Each operation runs for a fixed number of iterations and the loop reports the operation name,
//! the measured time per operation, and the operations per second. Signing with the stateless
//! hash-based scheme is slow, so those loops use a small iteration count. The measured numbers
//! calibrate the gas for the native post-quantum opcodes and the validator resource budget.
